/*
package com.desofs.backend.domain.valueobjects;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

class ImageUrlTest {

    @Test
    @DisplayName("Test valid URL")
    void testValidUrl() {
        String validUrl = "https://example.com/image.jpg";
        ImageUrl imageUrl = ImageUrl.create(validUrl);
        assertEquals(validUrl, imageUrl.getUrl());
    }

    @Test
    @DisplayName("Test null URL")
    void testNullUrl() {
        assertThrows(NullPointerException.class, () -> ImageUrl.create(null));
    }

    @Test
    @DisplayName("Test empty URL")
    void testEmptyUrl() {
        assertThrows(IllegalArgumentException.class, () -> ImageUrl.create(""));
    }

    @Test
    @DisplayName("Test URL with invalid characters")
    void testUrlWithInvalidCharacters() {
        String invalidUrl = "https://example.com/image<>.jpg"; // Invalid characters "<>"
        assertThrows(IllegalArgumentException.class, () -> ImageUrl.create(invalidUrl));
    }

}*/
